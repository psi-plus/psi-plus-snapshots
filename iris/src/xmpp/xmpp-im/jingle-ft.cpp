...
namespace Jingle {
    namespace FileTransfer {
        ...
        QList<Hash> File::hashes() const
        {
            return d ? d->hashes : QList<Hash>();
        }
        ...
        QByteArray File::amplitudes() const
        {
            return d ? d->amplitudes : QByteArray();
        }
        ...
        void File::setAmplitudes(const QByteArray &amplitudes)
        {
        ...
        {
            return d->file.isValid() && d->contentName.size() > 0 && (d->senders == Origin::Initiator || d->senders == Origin::Responder);
        }
