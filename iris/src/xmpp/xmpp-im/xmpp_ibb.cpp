...
IBBConnection::IBBConnection(IBBManager *m) : BSConnection(m)
{
    d            = new Private;
    d->m         = m;
    d->j         = nullptr;
...
void IBBConnection::setPacketSize(int blockSize) { d->blockSize = blockSize; }
...
#ifdef IBB_DEBUG
    qDebug("IBBConnection[%d]: accepting %s [%s]", d->id, qPrintable(d->peer.full()), qPrintable(d->sid));
#endif
...
int IBBConnection::state() const { return d->state; }

Jid IBBConnection::peer() const { return d->peer; }

QString IBBConnection::sid() const { return d->sid; }
...
void IBBConnection::waitForAccept(const Jid &peer, const QString &iq_id, const QString &sid, int blockSize,
...
#ifdef IBB_DEBUG
            qDebug("IBBConnection[%d]: %s [%s] accepted.", d->id, qPrintable(d->peer.full()), qPrintable(d->sid));
#endif
            d->state = Active;
            setOpenMode(QIODevice::ReadWrite);
            d->m->link(this);
...
IBBManager::IBBManager(Client *parent) : BytestreamManager(parent)
{
    ...
    connect(d->ibb, SIGNAL(incomingRequest(Jid, QString, QString, int, QString)),
            SLOT(ibb_incomingRequest(Jid, QString, QString, int, QString)));
    connect(d->ibb, SIGNAL(incomingData(Jid, QString, IBBData, Stanza::Kind)),
            SLOT(takeIncomingData(Jid, QString, IBBData, Stanza::Kind)));
    connect(d->ibb, SIGNAL(closeRequest(Jid, QString, QString)), SLOT(ibb_closeRequest(Jid, QString, QString)));
}
...
const char *IBBManager::ns() { return IBB_NS; }

Client *IBBManager::client() const { return d->client; }

BSConnection *IBBManager::createConnection() { return new IBBConnection(this); }
...
void IBBManager::ibb_incomingRequest(const Jid &from, const QString &id, const QString &sid, int blockSize,
                                     const QString &stanza)
{
...
void IBBManager::takeIncomingData(const Jid &from, const QString &id, const IBBData &data, Stanza::Kind sKind)
{
...
void IBBManager::ibb_closeRequest(const Jid &from, const QString &id, const QString &sid)
{
...
const char *IBBManager::sidPrefix() const { return "ibb_"; }

void IBBManager::link(IBBConnection *c) { d->activeConns.append(c); }

void IBBManager::unlink(IBBConnection *c) { d->activeConns.removeAll(c); }

IBBConnection *IBBManager::findConnection(const QString &sid, const Jid &peer) const
{
    for (IBBConnection *c : std::as_const(d->activeConns)) {
...
void IBBManager::doAccept(IBBConnection *c, const QString &id) { d->ibb->respondAck(c->peer(), id); }

void IBBManager::doReject(IBBConnection *c, const QString &id, Stanza::Error::ErrorCond cond, const QString &str)
{
...
JT_IBB::JT_IBB(Task *parent, bool serve) : Task(parent)
{
    d        = new Private;
    d->serve = serve;
}

JT_IBB::~JT_IBB() { delete d; }
...
void JT_IBB::respondAck(const Jid &to, const QString &id) { send(createIQ(doc(), "result", to.full(), id)); }

void JT_IBB::onGo() { send(d->iq); }
...
bool JT_IBB::take(const QDomElement &e)
{
    if (d->serve) {
        ...
        if (!openEl.isNull() && openEl.namespaceURI() == IBB_NS) {
            emit incomingRequest(Jid(from), id, openEl.attribute("sid"), openEl.attribute("block-size").toInt(),
...
Jid JT_IBB::jid() const { return d->to; }

int JT_IBB::mode() const { return d->mode; }

int JT_IBB::bytesWritten() const { return d->bytesWritten; }
