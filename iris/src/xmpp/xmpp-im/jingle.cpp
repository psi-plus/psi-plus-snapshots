...
namespace XMPP { namespace Jingle {
    ...
    public:
        ...
        // QMap<QString,Application*> myContent;     // content::creator=(role ==
        // Session::Role::Initiator?initiator:responder) QMap<QString,Application*> remoteContent; //
        // content::creator=(role == Session::Role::Responder?initiator:responder)
        QMap<ContentKey, Application *> contentList;
        QSet<Application *>             signalingContent;
        QHash<Action, OutgoingUpdate>
                outgoingUpdates; // session level updates. session-info for example or some rejected apps
        QString sid;
        ...
        void sendJingle(Action action, QList<QDomElement> update,
                        std::function<void()> successCB = std::function<void()>())
        {
        ...
        void doStep()
        {
            ...
            if (state == State::PrepareLocalOffer) { // we are going to send session-initiate/accept (already accepted
        ...
        std::tuple<AddContentError, Reason::Condition, Application *> parseContentAdd(const QDomElement &ce)
        {
            ...
            ContentBase c(ce);
            if (!c.isValid() || descriptionEl.isNull() || transportEl.isNull() || descriptionNS.isEmpty()
                || transportNS.isEmpty()) {
            ...
            if (!appPad) {
                return result { Unsupported, Reason::UnsupportedApplications,
                                nullptr }; // <unsupported-applications/> condition
            }
            ...
            if (descErr == Application::IncompatibleParameters) {
                ...
                if (!trPad) {
                    return result {
                        Unsupported, Reason::UnsupportedTransports, app.take()
                    }; // <unsupported-transports/> condition or we can try fallback and fail with <failed-transport/>
                }
            ...
            if (rejectSet.size()) {
                QTimer::singleShot(0, q, [this, rejectSet]() mutable {
                    ...
                    outgoingUpdates.insert(Action::ContentRemove,
                                           OutgoingUpdate { rejects, [this, rejects]() {
                                                               for (auto &r : rejects) {
                                                                   ContentBase c(r);
        ...
        bool handleIncomingTransportReplace(const QDomElement &jingleEl)
        {
            ...
            for (QDomElement ce = jingleEl.firstChildElement(contentTag); !ce.isNull();
                ...
                // if transport recovery is in progress and I as an inititiator started it then forbid remote
                // transport-replace
            ...
            for (auto &v : passed) {
                ...
                if (!app->incomingTransportReplace(
                        transport)) { // app should generate transport accept eventually. content-accept will work too
                                      // if the content wasn't accepted yet
                    toReject.append(ce);
                }
            }
        ...
        bool handleIncomingTransportAccept(const QDomElement &jingleEl)
        {
            ...
            for (QDomElement ce = jingleEl.firstChildElement(contentTag); !ce.isNull();
                ...
                Application *app = contentList.value(ContentKey { cb.name, cb.creator });
                if (!app || !app->transport() || app->transport()->pad()->ns() != transportNS
                    || !app->incomingTransportAccept(transportEl)) {
                    lastError = XMPP::Stanza::Error(XMPP::Stanza::Error::Cancel, XMPP::Stanza::Error::BadRequest);
                    return false;
                }
            }
    ...
    Jid Session::me() const { return d->manager->client()->jid(); }
