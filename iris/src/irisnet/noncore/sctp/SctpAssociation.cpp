...
inline static int onRecvSctpData(struct socket * /*sock*/, union sctp_sockstore /*addr*/, void *data, size_t len,
                                 struct sctp_rcvinfo rcv, int flags, void *ulpInfo)
{
    ...
    isDataChannel(isDataChannel)
{
    ...
    this->socket = usrsctp_socket(AF_CONN, SOCK_STREAM, IPPROTO_SCTP, onRecvSctpData, onSendSctpData,
                                  SendBufferThreshold, reinterpret_cast<void *>(this->id));
...
void SctpAssociation::TransportConnected()
{
    ...
    try {
        ...
        ret = usrsctp_setsockopt(this->socket, IPPROTO_SCTP, SCTP_PEER_ADDR_PARAMS, &peerAddrParams,
                                 sizeof(peerAddrParams));
...
void SctpAssociation::FillJson(QJsonObject &jsonObject) const
{
    MS_TRACE();

    // Add port (always 5000).
    jsonObject[QLatin1String("port")] = 5000;

    // Add OS.
    jsonObject[QLatin1String("OS")] = this->os;

    // Add MIS.
    jsonObject[QLatin1String("MIS")] = this->mis;

    // Add maxMessageSize.
    jsonObject[QLatin1String("maxMessageSize")] = int(this->maxSctpMessageSize);

    // Add sendBufferSize.
    jsonObject[QLatin1String("sendBufferSize")] = qint64(this->sctpSendBufferSize);

    // Add sctpBufferedAmountLowThreshold.
    jsonObject[QLatin1String("sctpBufferedAmount")] = qint64(this->sctpBufferedAmount);

    // Add isDataChannel.
    jsonObject[QLatin1String("isDataChannel")] = this->isDataChannel;
}
...
void SctpAssociation::SendSctpMessage(RTC::DataConsumer *dataConsumer, uint32_t ppid, const uint8_t *msg, size_t len,
                                      onQueuedCallback *cb)
{
    MS_TRACE();
...
void SctpAssociation::ResetSctpStream(uint16_t streamId, StreamDirection direction)
{
    ...
    if (ret == 0) {
        ...
        MS_WARN_TAG(sctp, "could not retrieve whether stream reconfiguration has been negotiated: %s\n",
                    std::strerror(errno));
...
void SctpAssociation::AddOutgoingStreams(bool force)
{
    ...
    int ret
        = usrsctp_setsockopt(this->socket, IPPROTO_SCTP, SCTP_ADD_STREAMS, &sas, static_cast<socklen_t>(sizeof(sas)));
...
void SctpAssociation::OnUsrSctpReceiveSctpNotification(union sctp_notification *notification, size_t len)
{
    ...
    case SCTP_ADAPTATION_INDICATION: {
        MS_DEBUG_TAG(sctp, "SCTP adaptation indication [%x]", notification->sn_adaptation_event.sai_adaptation_ind);
    ...
    case SCTP_ASSOC_CHANGE: {
        ...
        case SCTP_COMM_UP: {
            MS_DEBUG_TAG(sctp, "SCTP association connected, streams [out:%" PRIu16 ", in:%" PRIu16 "]",
        ...
        case SCTP_RESTART: {
            MS_DEBUG_TAG(sctp, "SCTP remote association restarted, streams [out:%" PRIu16 ", int:%" PRIu16 "]",
                         notification->sn_assoc_change.sac_outbound_streams,
                         notification->sn_assoc_change.sac_inbound_streams);
    ...
    case SCTP_REMOTE_ERROR: {
        ...
        MS_WARN_TAG(sctp, "remote SCTP association error [type:0x%04x, data:%s]",
                    notification->sn_remote_error.sre_error, buffer);
    ...
    case SCTP_STREAM_RESET_EVENT: {
        ...
        if (MS_HAS_DEBUG_TAG(sctp)) {
            ...
            MS_DEBUG_TAG(sctp, "SCTP stream reset event [flags:%x, i|o:%s|%s, num streams:%" PRIu16 ", stream ids:%s]",
                         notification->sn_strreset_event.strreset_flags, incoming ? "true" : "false",
                         outgoing ? "true" : "false", numStreams, streamIds.c_str());
        }
    ...
    case SCTP_STREAM_CHANGE_EVENT: {
        if (notification->sn_strchange_event.strchange_flags == 0) {
            MS_DEBUG_TAG(sctp, "SCTP stream changed, streams [out:%" PRIu16 ", in:%" PRIu16 ", flags:%x]",
            ...
            MS_WARN_TAG(sctp, "SCTP stream change denied, streams [out:%" PRIu16 ", in:%" PRIu16 ", flags:%x]",
                        notification->sn_strchange_event.strchange_outstrms,
                        notification->sn_strchange_event.strchange_instrms,
                        notification->sn_strchange_event.strchange_flags);
            ...
            MS_WARN_TAG(sctp, "SCTP stream change failed, streams [out:%" PRIu16 ", in:%" PRIu16 ", flags:%x]",
                        notification->sn_strchange_event.strchange_outstrms,
                        notification->sn_strchange_event.strchange_instrms,
                        notification->sn_strchange_event.strchange_flags);
    ...
    default: {
        MS_WARN_TAG(sctp, "unhandled SCTP event received [type:%" PRIu16 "]", notification->sn_header.sn_type);
    }
    }
}
