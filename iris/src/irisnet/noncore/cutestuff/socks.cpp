...
void SocksUDP::write(const QByteArray &data) { d->sd->writeDatagram(data, d->routeAddr, d->routePort); }
...
static int spc_get_version(QByteArray &from, SPCS_VERSION *s)
{
    ...
    memcpy(s->methodList.data(), a.data() + 2, size_t(num));
    return 1;
}
...
static int sps_get_version(QByteArray &from, SPSS_VERSION *s)
{
    ...
    s->version   = static_cast<unsigned char>(a[0]);
    s->method    = static_cast<unsigned char>(a[1]);
    return 1;
}
...
static QByteArray spc_set_authUsername(const QByteArray &user, const QByteArray &pass)
{
    ...
    a[1] = len1;
    memcpy(a.data() + 2, user.data(), size_t(len1));
    a[2 + len1] = len2;
    memcpy(a.data() + 3 + len1, pass.data(), size_t(len2));
    return a;
}
...
static int spc_get_authUsername(QByteArray &from, SPCS_AUTHUSERNAME *s)
{
    if (from.size() < 1)
        return 0;
    unsigned char ver = static_cast<unsigned char>(from.at(0));
    if (ver != 0x01)
        return -1;
    if (from.size() < 2)
        return 0;
    unsigned char ulen = static_cast<unsigned char>(from.at(1));
    if ((int)from.size() < ulen + 3)
        return 0;
    unsigned char plen = static_cast<unsigned char>(from.at(ulen + 2));
...
static QByteArray sp_set_request(const QString &host, quint16 port, unsigned char cmd1)
{
    ...
    a.resize(at + hlen + 1);
    a[at++] = hlen;
    memcpy(a.data() + at, h.data(), size_t(hlen));
    at += hlen;
...
static int sp_get_request(QByteArray &from, SPS_CONNREQ *s)
{
    ...
    if (atype == 0x01) {
        ...
        if ((int)from.size() < full_len)
            return 0;
        auto host_len = static_cast<unsigned char>(from.at(4));
        full_len += host_len;
        if ((int)from.size() < full_len)
            return 0;
        QByteArray cs;
    ...
    s->version      = static_cast<unsigned char>(a[0]);
    s->cmd          = static_cast<unsigned char>(a[1]);
...
SocksClient::SocksClient(QObject *parent) : ByteStream(parent)
{
    init();
...
SocksClient::SocksClient(QTcpSocket *s, QObject *parent) : ByteStream(parent)
{
    init();
...
QAbstractSocket *SocksClient::abstractSocket() const { return d->sock.abstractSocket(); }
...
bool SocksClient::isIncoming() const { return d->incoming; }
...
#endif
    d->sock.connectToHost(d->host, quint16(d->port));
}
...
qint64 SocksClient::bytesAvailable() const { return ByteStream::bytesAvailable(); }
...
#endif
    d->step        = StepRequest;
    auto       cmd = d->udp ? REQ_UDPASSOCIATE : REQ_CONNECT;
    QByteArray buf;
    ...
    if (d->step == StepVersion) {
        ...
        if (r == -1) {
            ...
            for (int n = 0; n < (int)s.methodList.size(); ++n) {
                unsigned char c = static_cast<unsigned char>(s.methodList[n]);
                if (c == 0x00)
                    methods |= AuthNone;
                else if (c == 0x02)
...
void SocksClient::grantUDPAssociate(const QString &relayHost, quint16 relayPort)
{
    if (d->step != StepRequest || !d->waiting)
        return;
...
QHostAddress SocksClient::peerAddress() const { return d->sock.peerAddress(); }

quint16 SocksClient::peerPort() const { return d->sock.peerPort(); }

QString SocksClient::udpAddress() const { return d->udpAddr; }

quint16 SocksClient::udpPort() const { return d->udpPort; }
...
bool SocksServer::isActive() const { return d->serv->isListening(); }
...
int SocksServer::port() const { return d->serv ? d->serv->serverPort() : 0; }

QHostAddress SocksServer::address() const { return d->serv ? d->serv->serverAddress() : QHostAddress(); }
...
void SocksServer::writeUDP(const QHostAddress &addr, quint16 port, const QByteArray &data)
{
    if (d->sd) {
...
{
    while (d->sd->hasPendingDatagrams()) {
        QByteArray   datagram(int(d->sd->pendingDatagramSize()), Qt::Uninitialized);
        QHostAddress sender;
        quint16      senderPort;
