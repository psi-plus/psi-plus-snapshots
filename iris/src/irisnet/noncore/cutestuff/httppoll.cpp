...
HttpPoll::HttpPoll(QObject *parent) : ByteStream(parent)
{
    d = new Private(this);
...
QAbstractSocket *HttpPoll::abstractSocket() const { return d->http.abstractSocket(); }
...
void HttpPoll::connectToUrl(const QUrl &url) { connectToHost("", 0, url); }
...
QByteArray HttpPoll::makePacket(const QString &ident, const QString &key, const QString &newkey,
...
int HttpPoll::pollInterval() const { return d->polltime; }
...
void HttpProxyPost::setUseSsl(bool state) { d->useSsl = state; }

QAbstractSocket *HttpProxyPost::abstractSocket() const { return d->sock.abstractSocket(); }
...
void HttpProxyPost::stop() { resetConnection(); }

QByteArray HttpProxyPost::body() const { return d->body; }
...
#endif
    resetConnection(true);
    emit error(ErrConnectionRefused); // FIXME: bogus error
}
...
HttpProxyGetStream::HttpProxyGetStream(QObject *parent) : QObject(parent)
{
    d      = new Private(this);
    d->tls = nullptr;
    connect(&d->sock, SIGNAL(connected()), SLOT(sock_connected()));
...
int HttpProxyGetStream::length() const { return d->length; }
