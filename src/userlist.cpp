	...
	if(!userResourceList().isEmpty()) {
		...
		for(UserResourceList::ConstIterator rit = srl.begin(); rit != srl.end(); ++rit) {
			...
			if(r.status().mucItem().jid().isEmpty())
				str += hr + QString("<div style='white-space:pre'><%1=\"%2\"> ").arg(imgTag).arg(istr) + QString("<b>%1</b> ").arg(Qt::escape(name)) \
				+ QString("(%1)").arg(r.priority()) + secstr + "</div>";
			else
				str += hr +  QString("<div style='white-space:pre'><%1=\"%2\"> ").arg(imgTag).arg(istr) + QString("<b>%1</b> ").arg(Qt::escape(name)) \
				+ QString("(%1) &lt;%2&gt;").arg(r.priority()).arg(JIDUtil::toString(r.status().mucItem().jid(),true)) + secstr + "</div>";
